...
document_id: "f69cd1cf496babed02d19c45f458c6dfd243833cc68d2f3b27ae9fea6ace5fc3"
company_key: "yc-doublehq"
company: "Double"
source_id: "yc-doublehq-rss-72f065f06a12"
canonical_url: "https://doublehq.com/blog/what-is-trial-balance-accounting/"
published_at: "2026-07-27T18:01:03+00:00"
first_seen_at: "2026-07-27T19:27:34.117278+00:00"
fetched_at: "2026-07-28T20:32:04.512542+00:00"
content_hash: "sha256:861b9517c31fbfe52fe6635e3c915c6a31839f6fdff20294ac5b4ac5eeddbe52"
---

# What Is a Trial Balance? Purpose, Format, and Examples (July 2026)
...
Every transaction touches at least two accounts, one debit and one credit of equal value. Record a $500 utility payment, and cash drops by $500 while utilities expense rises by $500. That symmetry is the whole premise of[double-entry bookkeeping](https://www.investopedia.com/terms/d/double-entry.asp) , and the trial balance is where you check whether it held up across every account, all period long.


The general ledger and the trial balance serve distinct purposes. The ledger holds the detail: every transaction, dated and coded to a specific account.[General ledger reconciliation](https://doublehq.com/blog/general-ledger-reconciliation/) is a separate process that validates those ledger balances against supporting sources. The trial balance strips that detail away and shows only the ending balance for each account, summed into two columns.
...
Picture a small firm, Bright Leaf Consulting, closing its books as part of the[month-end close process](https://doublehq.com/blog/month-end-close-process-guide/) . Pulling every account balance from the ledger produces this trial balance.
...
A[bank reconciliation](https://doublehq.com/blog/bank-reconciliation-complete-guide-with-examples/) and subsidiary ledger reviews remain a necessary second layer of checking, since a trial balance only confirms mathematical consistency, not factual accuracy. Understanding[reconciliation in finance](https://doublehq.com/blog/reconciliation-finance-definition/) clarifies why this second layer matters.
...
The report lists every account in the Chart of Accounts, including sub-accounts, so parent accounts and their children each show separate lines. You can customize columns, filter by accounting basis, and export to Excel or CSV for review outside the browser, which is useful when teams need a[bank reconciliation statement format in Excel](https://doublehq.com/blog/bank-reconciliation-statement-format-excel/) alongside the trial balance.
...
3. Supports adjusting entries. Reviewing balances at period end surfaces missing accruals, deferrals, and depreciation that need correcting before the books close, and flags the kinds of discrepancies that[variance accounting analysis](https://doublehq.com/blog/variance-accounting-guide/) is designed to explain.
4. Prepares for audits. Internal reviewers and external auditors both start with the trial balance, since it maps every account balance in one place for testing, and[flux analysis](https://doublehq.com/blog/flux-analysis-explained-complete-guide/) is often the next step to explain period-over-period movement.
5. Diagnoses migration issues. When moving to new accounting software, comparing trial balances before and after the switch confirms that opening balances carried over without corruption, a check that pairs well with[month-end close automation](https://doublehq.com/blog/month-end-close-automation) tools that handle the transition systematically.
