...
document_id: "505578c881b9a3fb502515a9184191602ec698e663d830f1e3df574edc086247"
...
content_hash: "sha256:e6a88078d3bae09aef7bcb58bc7a3aea90424fda74c6df676e4799ef78c8582b"
---

# Substitute YAML with Nouns and Verbs in CI/CD Pipelines

*My name is*[Sebastian Huckleberry](https://www.linkedin.com/in/sebastiantiedtke/) *, I am the CEO at Stateful, we make*[Runme](https://runme.dev/) *, and I’m excited to contribute this guest blog post. Let’s dive right in.*A few weeks back, the folks at Dagger released their[Container-Native Shell](https://dagger.io/blog/a-shell-for-the-container-age-introducing-dagger-shell) , and it’s awesome! Leading up to the launch, the team at Stateful/Runme was invited for a sneak preview and decided to build a frontend for Dagger Shell into[Runme](https://runme.dev/) .
...
```text
dagger   -c   'github.com/runmedev/vscode-runme | build $(github.com/runmedev/runme |
link-release --version "latest" "linux/amd64") | integration-test | stdout'
```
...
```text
link-release --version "latest" "linux/amd64") | unit-test | stdout'
```
...
3. Docs are often out of sync or scattered. It’s challenging to keep things clear and close to the work, even in 2025.
...
We target the` ExtensionVsix` noun, which describes the VSIX as the final file format – our desired outcome. The **Noun** ‘nomenclature’ with capitalization is intentional because the result produced is a local extension file that we can install or ship to Microsoft’s VS Code marketplace.
...
Now, while **Nouns** describe Dagger artifacts (file, directory, container, etc), the Dagger functions performing operations are **verbs** (lowercase). If this feels reminiscent of software system design tasks, it is. It involves identifying the essential components of your domain, including nouns that represent entities and verbs that detail the related actions. Except for primarily serving comprehension in Software Design, we use the exact lingo to express Dagger pipelines.


Using Runme as a frontend, the idea is to use Dagger Shell pipes and references to decompose a complex pipeline into a series of concise **Dagger sentences** which consist of **Nouns** and **verbs** and, conveniently, have a descriptive name (yes, another **Noun** ). Then, we lean on Runme’s universal task runner to generate the Dagger Shell code required to execute pipelines dynamically, where the targeted **Noun** is the entry point. Too abstract? Here’s an example.


Consider the difference between the` ExtensionVsix` and` Extension` **Nouns** . The latter builds (Typescript compiler) the extension, linking it with the Kernel binary:
...
Just like in linguistics, **Nouns** can act as subjects or objects in ‘Dagger sentences’. Notice how` KernelBinary` is being passed using the Shell Expression syntax` $(...)` to make it the object of the` build` **verb** (the predicate in Linguistics), running the Dagger function on the subject` Extension` . Now there are also attributes (optional and positional arguments) which control the behavior of functions (aka **verbs** ), but let’s not get carried away with the language analogies.
...
To further illustrate the “Nouns and Verbs” concept, let’s add a new **Noun** to, for example, grab the screenshots taken during tests. Add a new` IntegrationTestScreenshots` that gets hold off the directory containing the WebDriver screenshots after the test suite has run.


```text
### Exported in runme.dev as IntegrationTestScreenshots
Extension | integration-test --runme-test-token RUNME_TEST_TOKEN | directory "logs/screenshots" | export /tmp/e2e-screenshots
```
