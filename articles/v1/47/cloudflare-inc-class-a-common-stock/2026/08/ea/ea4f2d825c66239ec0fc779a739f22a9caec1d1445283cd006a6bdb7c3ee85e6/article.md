...
document_id: "ea4f2d825c66239ec0fc779a739f22a9caec1d1445283cd006a6bdb7c3ee85e6"
company_key: "cloudflare-inc-class-a-common-stock"
company: "Cloudflare Inc."
source_id: "cloudflare-inc-class-a-common-stock-rss-72eb55cabac1"
canonical_url: "https://blog.cloudflare.com/agent-development-lifecycle/"
...
content_hash: "sha256:2c0e151aa819ccb0d22697f52b3f1d77748aafaae7a9a535936903dbc90c0940"
---

# The Agent Development Lifecycle has arrived on Cloudflare

Engineering managers spent the past few decades figuring out ways for many programmers to work together on a shared codebase. This work dates all the way back to the “Systems Development Lifecycle” ([RAND, 1975](https://www.rand.org/pubs/reports/R1855.html) ) - today commonly referred to as the “Software Development Lifecycle” (SDLC), which defines the following phases:
...
Cloudflare treats agents as our customers. They can[buy domains](https://blog.cloudflare.com/agents-stripe-projects/) , create[temporary accounts](https://blog.cloudflare.com/temporary-accounts/) and[use the entire Cloudflare API](https://blog.cloudflare.com/code-mode-mcp/) . We know that agents need APIs and tools to be able to manage the full SDLC on behalf of our customers — not just the start of it.
...
- [@cloudflare/ci](https://blog.cloudflare.com/ci-workflows) — a new way to run CI/CD across millions of repos, that can self-heal and spawn agents to do much more complex tasks, build on Cloudflare Workflows.
- [OpenTelemetry traces in local dev](https://blog.cloudflare.com/local-tracing) — giving agents the same observability they have in production, built into Wrangler and the Cloudflare Vite plugin.
- [Introducing: Cloudflare Agents and Agent Traces](http://blog.cloudflare.com/agents-on-cloudflare) — a new home for observing, maintaining and improving agents, centered around OpenTelemetry traces from agents.
- [How Cloudflare enforces engineering standards using AI](http://blog.cloudflare.com/engineering-standards-enforcement) — our own experience enforcing best practices across all of our products’ and systems’ repositories and specs.
- [How we built a software factory to drive Astro’s GitHub issue count to zero](https://blog.cloudflare.com/astro-issue-triage) — our own experience building systems to automatically triage, reproduce, verify and fix issues for a large and growing open source project.
...
Right now,[everyone](https://x.com/zachlloydtweets/status/2069789929073262945)[is](https://x.com/matanSF/status/2066578088184680920)[talking](https://x.com/dexhorthy/status/2081797628552270027)[about](https://x.com/bcherny/status/2077929390806073807)[building](https://x.com/gokulr/status/2032271386161684665) “software factories” — agent-driven systems that take input and autonomously build, improve, deploy and manage software. Take an input, whether it’s a production error, a bug report from a customer, or an idea for a new feature, and delegate it entirely to an agent.
...
Workflows go beyond a series of linear steps though. They can be[defined dynamically](https://blog.cloudflare.com/dynamic-workflows/) , and they can spawn agents or other Workflows.[This example](https://flueframework.com/docs/guide/workflows/#example-cloudflare-workflows) shows a Workflow that reviews new data from the past day. The Workflow has full control over when and how the agent is prompted, and can pass along context between steps:
...
Once you see this pattern, and are “Workflow-pilled” as Cloudflare is, you start to ask: what else could I have a Workflow handle for me? What other human-bottlenecked steps could I delegate to this combination of Workflow +[Flue agents](https://flueframework.com/) ?


## The full ADLC, on the Cloudflare stack


With[Workflows](https://developers.cloudflare.com/workflows/) able to orchestrate complex steps, and[Artifacts](https://developers.cloudflare.com/artifacts/) as the storage layer for code, when you look at the SDLC stages, everything an agent needs to own the whole process of building, shipping and maintaining software is on Cloudflare:
...
[Vite](https://developers.cloudflare.com/workers/vite-plugin/) ,[Rolldown](https://rolldown.rs/) , and[Oxc](https://oxc.rs/) — the fastest toolchain for your agent
[Local dev for everything](https://developers.cloudflare.com/workers/local-development/) — what your agent sees locally, is the same runtime and environment that will run in production
[Local Explorer](https://developers.cloudflare.com/workers/local-development/local-explorer/) ,[Local Traces](https://blog.cloudflare.com/local-tracing/) — your agent has the same APIs to debug locally as it does in production
[Remote bindings](https://developers.cloudflare.com/changelog/post/2025-09-16-remote-bindings-ga/) — let agents run code locally, while using real production resources running on Cloudflare
[Preview URLs](https://developers.cloudflare.com/workers/versions-and-deployments/preview-urls/) — give every pull request a preview for the agent to validate and use


Test


[Browser Run](https://developers.cloudflare.com/browser-run/) — programmable headless browsers in the cloud
[Vitest](https://developers.cloudflare.com/workers/testing/vitest-integration/) — run tests in the Workers runtime


Deploy


[Flagship](https://blog.cloudflare.com/flagship/) — every change gets its own feature flag
[Gradual Deployments](https://developers.cloudflare.com/workers/versions-and-deployments/gradual-deployments/) — roll out code changes to a percentage of traffic, ramp up over time
...
[Workers Logs](https://developers.cloudflare.com/workers/observability/logs/) — let agents tail live logs or query adhoc to identify issues to automatically fix
[Agent Traces](http://blog.cloudflare.com/agents-on-cloudflare) — capture every agent session and use it to improve
[Cloudflare MCP Server](https://blog.cloudflare.com/code-mode-mcp/) — powered by Code Mode and[Dynamic Workers](https://blog.cloudflare.com/dynamic-workers/)[Analytics Engine](https://developers.cloudflare.com/analytics/analytics-engine/) — high cardinality analytics built on Clickhouse, to let agents query who is using what
