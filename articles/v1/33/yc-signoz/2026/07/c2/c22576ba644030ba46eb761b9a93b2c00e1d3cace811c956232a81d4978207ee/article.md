...
document_id: "c22576ba644030ba46eb761b9a93b2c00e1d3cace811c956232a81d4978207ee"
company_key: "yc-signoz"
company: "SigNoz"
source_id: "yc-signoz-rss-564a62b873f8"
canonical_url: "https://signoz.io/guides/how-to-log-all-the-processes-running-inside-a-docker-container"
published_at: "2026-07-07T00:00:00+00:00"
first_seen_at: "2026-07-20T23:20:42.602972+00:00"
fetched_at: "2026-07-28T20:47:27.048275+00:00"
content_hash: "sha256:db307a9526e2233d1ed058d18cbfdff2a4d2d0948fa4ab90a0f88526edfa6d62"
---
...
There are several approaches to logging processes in[Docker containers](https://signoz.io/blog/docker-logs/) . Let's explore the most common and effective methods. Do note that for the container's own stdout/stderr stream rather than its processes, the[Docker view logs guide](https://signoz.io/guides/docker-view-logs/) covers the base commands.
...
Access the container's shell: To enter a container and execute commands, use` docker exec.`
...
` /bin/bash` provides an interactive shell interface and script execution environment within Docker containers, enabling command execution and process control.


2.


Run the 'ps' command: Once inside the container, execute the following:
...
The` ps -eaf` command provides a comprehensive view of all processes running inside the Docker container, including detailed process information, which aids in monitoring and performance optimization.
...
Configure[Docker logging driver](https://signoz.io/blog/docker-logging/) : When starting a container or globally, you can configure a specific logging driver through the[Docker daemon](https://signoz.io/guides/docker-daemon-logs/) configuration file (` /etc/docker/daemon.json` ). For example, to use the` json-file` logging driver with rotation options:
...
Restart Docker daemon: After modifying the configuration, restart the Docker service to apply the changes:
...
View logs: To view the logs from a running container, use the` docker logs` command:
...
This command will display the logs from the container’s standard output and standard error streams.
...
Modify your Dockerfile: Next, update your Dockerfile to include the wrapper script and ensure it's executable.
...
Build and run your container: Now that the Dockerfile is set up, build and run your container.


-


Build the Docker image: This command is used to build an image from a Dockerfile in the current directory` (.)` . The` -t` flag tags the image with a name. Here,` <my-logging-container>` is a placeholder for the image name.
...
Accessing the Logs The container logs processes every minute into the` /var/log/container_processes.log` file. To access the log file:
...
Update the Dockerfile: Modify the` Dockerfile` to include Supervisor and configure it to manage your application:
...
Create a Supervisor Configuration File: Create a` supervisord.conf` file in the same directory as your Dockerfile to configure Supervisor to manage your Python app.
...
Where to See Supervisor Logs: You can access Supervisor logs directly within the container:
...
Using a process manager like Supervisor is particularly useful in environments requiring high reliability and more granular control over process execution within containers.
...
Choose a Custom Logging Library: For this example, we'll use` loguru` , a popular logging library that offers a more flexible and user-friendly setup than Python's built-in` logging` module.


3.


Update Your Application Code: Integrate the custom logging library into your application code. For this example, we will add it to our` app.py` file.


1.


Install` loguru:` Ensure` loguru` is installed via pip:
...
Update Your Dockerfile: Modify your Dockerfile to include the custom logging library and update your application to use it.
...
Build and Run the Docker Container: Rebuild your Docker image and run the container:
...
1. Accessing Logs: To view the logs generated by` loguru` , Access container logs:
...
Integrating logging libraries into your applications is a good approach for teams that need precise control over their logs and want to capture application-specific data.
